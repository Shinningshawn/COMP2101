...
test -r /etc/resolv.conf && echo "/etc/resolv.conf is readable by you" || echo "/etc/resolv.conf is not readable by you"
test -w /etc/resolv.conf && echo "/etc/resolv.conf is writable by you" || echo "/etc/resolv.conf is not writable by you"
test -x /etc/resolv.conf && echo "/etc/resolv.conf is exacutable by you" || echo "/etc/resolv.conf is not executable by you"
...
[ -r /tmp ] && echo "/tmp is readable" || echo "/tmp is not readable by you"
[ -w /tmp ] && echo "/temp is writable" || echo "/tmp is not writable by you"
[ -x /tmp ] && echo "/tmp is accessable" || echo "/tmp is not accessable by you"
...
firstNumber=4
secondNumber=7
read -p "Please provide the first number: " firstNumber
read -p "Please provide the second number: " secondNumber
...
[ -z $USER ] && echo "the variable SHELL has content" || echo "The variable SHELL is empty"
